...
class AES_Step : public AES
{
    using Block = array<unsigned char, 16>;
private:
    Block _key_cache[20];
    Block _state;
    static Block aes_128_key_expansion(Block key, char keygened){
//        auto begin = key.begin() + 12;
        array<unsigned char, 4> g;
        copy(key.begin()+13, key.end(), g.begin());
        g[3] = key[12];
        for(auto &b : g) {
            b = sbox[b];
        }
    ...
    void aes128_load_key(string enc_key){
        ...
        copy(enc_key.begin(), enc_key.begin() + 16, _key_cache[0].begin());
        for(int i = 1; i < 11; ++i) {
            _key_cache[i] = aes_128_key_expansion(_key_cache[i - 1], rcon[i]);
        }
        for(int i = 1; i < 10; ++i) {
            _key_cache[10 + i] = inv_mix_columns(_key_cache[10 - i]);
        }
    }
private:
    void sub_bytes(){
    ...
    Block mix_columns(const Block block){
        Block tmp;
    ...
    Block inv_mix_columns(const Block &block){
        Block tmp;
...
public:
    AES_Step(QObject* parent = Q_NULLPTR):AES(parent), _state{0} {}
    AES_Step(const string & key, QObject* parent = Q_NULLPTR):AES(parent), _state{0} {
        set_key(key);
    }
    ~AES_Step() override{}
    void set_state(char * block) override {
        copy(block, block + 16, _state.begin());
    }
    string get_state() const override {
        return string(_state.begin(), _state.end());
    }
